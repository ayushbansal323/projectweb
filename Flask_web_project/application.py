...
from Flask_web_project.cryptoProject.sqlinject import checkRegMatch
from Flask_web_project.cryptoProject.xssdetection import check_xss_attack
...
def login():
	...
	if checkRegMatch(username) or checkRegMatch(password):
		note="attempt for SQL Injection detected"
		notetype="danger"
	...
	if db.execute("SELECT * FROM register WHERE username=:username" , {"username":username}).rowcount == 0:	
		...
		return render_template("index.html",note=note,notetype=notetype)
	#sql injection
	register=db.execute("SELECT * FROM register WHERE username=:username AND password='"+password+"'" , {"username":username} ).first()
	if register is None: 
...
def page_not_found(e):
	if check_xss_attack(request.path):
		return "Attempt for xss attack detected"
