...
class TestHome(CreateObjects, TestBase):
	...
	def test_search_by_isbn(self):
		...
		select = Select(self.driver.find_element_by_id('submitSelector'))
		select.select_by_visible_text('isbn')
	...
	def test_click_book(self):
		self.login_user()
		self.driver.find_element_by_xpath("//a[@href = '/book/039925675X']").click()
		time.sleep(8)
		assert url_for('book/039925675X') in self.driver.current_url
